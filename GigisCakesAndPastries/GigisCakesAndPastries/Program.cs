using GigisCakesAndPastries;

public class Program
{
    public static void Main(string[] args)
    {
        //Database.Admins.Clear();
        //Database.Customers.Clear();
        //Database.CreateFileToGoogleDrive();

        //DateTime dt = new DateTime(2002, 09, 09);

        /*Admin rene = new Admin("00-00", "de los Reyes", "Rene Angelo", "Gonzalo", "rendzdelosreyes@gmail.com", "rendznicoy", "sadude75","09991980065", "Ormoc City", 1, 9, 2002, DateTime.Now);

        Database.Admins.Add(rene);

        Database.SerializeAdmin();
        Database.UploadAdminList();*/

        /*Customer customers = new Customer("01-00", "de los Reyes", "Rene", "Vanzuela", "renevanzueladelosreyes@gmail.com", "rene", "babyzach1", "09617501808", "Ormoc City", 1, 22, 1950, DateTime.Now);

        Database.Customers.Add(customers);*/

        //Database.SerializeCustomers();
       // Database.UploadCustomerList();
    }
}